...
while year >= 2002:
    ...
    for t in all_url_jpg:
        ...
        fpath_sex = 'D:\\pa_JPG2\\'+str(mm[i*7+4+6]).rstrip()
        fpath_xueyuan = fpath_sex+'\\'+str(mm[i*7+6+6]).rstrip()
        ...
        print i
        print str(mm[i*7+3+6]).rstrip()
        print str(mm[i*7+6+6]).rstrip()
        print t
        
        f = open(fpath_xueyuan+'\\'+str(mm[i*7+2+6])+'+'+str(mm[i*7+3+6]).rstrip()+'.jpg',"wb+")
